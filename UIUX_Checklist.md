...
- [ ] HUD에 Players, Time, 술래 정보가 표시된다.
- [ ] 게임 상태에 따라 Players, Time, 술래 값이 변경된다.
...
- [ ] 화면 전환을 테스트했다.
- [ ] UI 입력을 테스트했다.
- [ ] Players, Time, 술래 데이터 연결을 확인했다.
- [ ] 게임 종료 후 HUD가 정상적으로 초기화되는 것을 확인했다.
...
- 기대 결과: FIND 버튼을 누르면 Players가 감소하고 술래 수가 증가해야 한다.
- 실제 결과: FIND 버튼을 누를 때마다 Players가 1씩 감소하고 술래 수가 1씩 증가했다.
- 수정 내용: GameHUD에 TaggerText와 taggerCount를 추가하고 FindPlayer()에서 두 값을 갱신하도록 연결했다.
- 다시 확인한 결과: Players와 술래 수가 게임 진행에 맞게 정상적으로 변경되었다.
...
| 뒤로 가기 | ESC |
| 정보 우선순위 | 플레이어 수, 술래 수, 남은 시간을 우선 표시 |
| 버튼 피드백 | 선택 시 색상 변화 |
...
숨바꼭질 게임의 특성상 체력 시스템은 필요하지 않다고 판단하여 제외하고, 플레이어 수와 술래 수, 남은 시간을 HUD의 주요 정보로 구성했다.

UI 입력은 마우스뿐만 아니라 방향키와 Enter, ESC를 사용할 수 있도록 Input System과 EventSystem을 구성했다.

실제 테스트 과정에서 Pause 메뉴의 위 방향키 이동이 동작하지 않는 문제가 발견되었으며, Button Navigation을 Explicit으로 설정하고 각 버튼의 이동 대상을 연결하여 수정했다.

또한 결과 화면에서 기존 GAME START 텍스트가 GAME CLEAR 화면과 겹치는 문제가 발생하여 게임 종료 시 GameTitle을 비활성화하도록 수정했다.

추가로 FIND 기능을 통해 Players와 술래 수가 실제 게임 상태에 따라 변경되도록 HUD 데이터를 연결했으며, 게임을 다시 시작하면 Players, Time, 술래 수가 초기값으로 돌아가는 것을 확인했다.